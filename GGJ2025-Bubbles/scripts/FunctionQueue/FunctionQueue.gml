...
///	Version: April 18th, 2024. https://github.com/JonathanHackerCG/GM-FunctionQueue

#region FunctionItem(function, [arguments], [owner], [temporary], [tag]);
/// @func FunctionItem(function, arguments, owner, temporary, tag):
/// @desc Constructor for FunctionItem.
/// @arg	{Function}						function
/// @arg	{Array}								[arguments]
/// @arg	{ID.Instance|Struct}	[owner]
/// @arg	{Bool}								[temporary]
/// @arg	{String}							[tag]
/// @returns {Struct.FunctionItem}
/// @context FunctionQueue
function FunctionItem(_function, _args = undefined, _owner = undefined, _temporary = undefined, _tag = undefined) constructor
{
	func	= _function;
	args	= _args;
	owner	= _owner;
	...
	#region call();
	/// @func call();
	/// @desc Calls the FunctionItem.
	/// If there is an owner assigned, and the owner exists, it will call in the owner's scope.
	static call = function()
	{
		var _func = func;
		if (owner != undefined)
		{
			if (instance_exists(owner) || is_struct(owner))
			{
				var _func = method(owner, func);
			}
		}
		__FunctionQueue_call_function_ext(_func, args);
	}
	#endregion
}
#endregion
#region FunctionQueue([owner], [persistent], [temporary]);
/// @func FunctionQueue():
...
function FunctionQueue(_owner = undefined, _persistent = false, _temporary = false) constructor
{
	...
	static __convert_func = function(_function, _owner)
	{
		...
		try
		{
			...
			if (instance_exists(_owner) || is_struct(_owner))
			{
	...
	/// @returns {Bool}
	/// @context FunctionQueue
	static update = function(_delay = false)
	{
		__index_next = 0;
		...
		else
		{
			...
			_done = bool(__FunctionQueue_call_function_ext(_func, _args) ?? true);
		}
	...
	{
		var _item = new FunctionItem(_func, _args, _owner, _temporary, _tag);
		item_insert(_pos, _item);
		return _item;
	}
	#endregion
	...
	#region item_insert(index, item);
	/// @desc Insert an item into the FunctionQueue at a index.
	/// @arg	{Real}								index
	/// @arg	{Struct.FunctionItem}	item
	/// @context FunctionQueue
	static item_insert = function(_pos, _item)
	{
		_item.func = __convert_func(_item.func, _item.owner);
		
		array_insert(__items, _pos, _item); __size++;
		if (_pos <= __index) { __index++; }
	}
	#endregion
	#region item_push(item);
	/// @desc Insert an item at the end of the FunctionQueue.
	/// @arg	{Struct.FunctionItem}	item
	/// @context FunctionQueue
	static item_push = function(_item)
	{
		item_insert(__size, _item);
	}
	#endregion
	#region item_interrupt(item);
	/// @desc Interrupt the current item with a new one. The original item will resume after this one.
	/// @arg	{Struct.FunctionItem}	item
	/// @context FunctionQueue
	static item_interrupt = function(_item)
	{
		item_insert(get_index(), _item);
		change_index(-1);
	}
	#endregion
	#region item_next(item);
	/// @desc Insert an item immediately after the current item.
	/// Calling this method repeatedly will add each new item after the previous, until the index changes.
	/// @arg	{Struct.FunctionItem}	item
	/// @context FunctionQueue
	static item_next = function(_item)
	{
		item_insert(__index + 1 + __index_next, _item);
		__index_next++;
	}
	#endregion
	
	#region get_item(index);
	/// @desc Returns an item (Struct.FunctionItem) at an index in the FunctionQueue.
...
#region __FunctionQueue_call_function_ext(function, params);
/// @desc Runs a function/method with an Array of parameters.
/// @arg	{Function} function
/// @arg	{Array} parameters
/// @returns {Any}
/// @context FunctionQueue
function __FunctionQueue_call_function_ext(_function, _args)
{
	...
	switch(_size)
	{
		...
		default: show_error("Too many parameters for __FunctionQueue_call_function_ext.", false); return false;
	}
}
#endregion